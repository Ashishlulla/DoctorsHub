...
namespace DoctorsHub.Web.Configurations
{
    public static class AddHttpClientExtension
    {
        public static IServiceCollection AddHttpClientServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddHttpClient<BusinessInsightsApiService>(client =>
            {
                client.BaseAddress = new Uri(configuration["MyAPI:BaseUrl"]!);
            });

            services.AddHttpClient<BillingApiService>(client =>
            {
                client.BaseAddress = new Uri(configuration["MyAPI:BaseUrl"]!);
            });
            ...
            services.AddHttpClient<DepartmentApiService>(client =>
            {
                client.BaseAddress = new Uri(configuration["MyAPI:BaseUrl"]!);
            });
